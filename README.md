...
I run a homelab within my house and even though its small, it is a blast to play with and I learn a lot from it. The following equipment is what I have in my homelab:
...
Within my homelab I run several different open-source apps, tools, and services. Most of them are deployed within my k3s cluster but some are deployed using docker or directly on the VM's. For example, I use docker to deploy and backup my pihole instances, while my postgresql database and Zabbix monitoring server are deployed directly on VM's. A few other points to note:
...
- I pay for and use [tailscale](https://tailscale.com/) (highly recommend) as my VPN to access my homelab from anywhere.
...
Finally, here is a screenshot showing a list of all the applications that are currently deployed in my homelab:

![Screenshot](https://www.dropbox.com/s/3evh1fj9hkn9xmg/homelab-services.jpg?dl=0)
